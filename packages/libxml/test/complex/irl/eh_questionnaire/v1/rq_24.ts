...
html() {
  ...
  $("/html/body//form[@name]//li[contains(@class, 'formActions')]") {
    $mw_submit_label = fetch("./input[@type='submit' and contains(@name, 'Exit')]/@value")
    
    $("./input[@type='submit' and contains(@name, 'Continue')]") {
      attribute("value", $mw_submit_label)
    }
    $("./input[@type='submit' and contains(@name, 'Exit')]") {
      remove()
    }
  }
}
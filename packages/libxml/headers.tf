...
" Removes the node specified by `%xpath_selector` - [click for example](http://console.moovweb.com/learn/tutorials/function_guides/removing). @example `remove(\"//table\")` will remove all table elements from the document. "
...
" Opens the attribute you select with **%name** - [click for example](http://console.moovweb.com/learn/tutorials/function_guides/attribute). @example `attribute(\"class\")` opens the class for modification. "

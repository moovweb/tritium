module Tritium::Engines::Reference::Scope
  module NodeModules 
    module ActionMethods
      def insert_at(position, tag_name, contents = "", attributes = {}, &block)
        if contents.is_a? Hash
          attributes = contents
          contents = ""
        end

        new_node = position_node(node, "<#{tag_name} />", position).first
      ...
      def inject_at(position, html, &block)
        result = position_node(node, html, position)
      ...
      def copy_here(selector, position = "bottom", &block)
        node.xpath(selector).each do |target|

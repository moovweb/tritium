...
module Tritium
  module Parser
    class ReaderInstruction
      ...
      # During reading, if this ReaderInstruction has an Arg that is an ReaderInstruction, that instruction
      # will be tracked by the parent ReaderInstruction as a child. We don't really want that, so this
      # is run recusively after reading is complete.
      def clean_args!
        children.each do |child|
          child.args.each do |arg|
            if arg.is_a?(ReaderInstruction)
              arg.is_arg = true
              # Deparent any args
              arg.clean_args!
            end

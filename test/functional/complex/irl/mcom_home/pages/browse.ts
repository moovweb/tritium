...
html() {
  ...
  $("/html/body[starts-with(@class,'browse')]|//body[starts-with(@class,'subSplash')]") {
    add_class("mw_browse")
    $("div[@id='doc3']/div[@id='bd']/div[@id='globalContentContainer']/div[@id='localContentContainer']/div[@id='flexOptionalAdBanner']"){
      remove()
    }
  }
  $("(html/body[contains(concat(' ', @class, ' '), ' splash ')])[1]") {
    add_class("mw_category")
    $("//a[contains(@href, '/store/corporate/index.jsp')]") {
      attribute("href","/store/corporate/index.jsp")
      # remove()
         }
  }
  ...
  #[["conditional_selector", "div[id^=\"thumbnails\"]"], ["negate", ""]]
    ...
    # No need to wrap the contents at all
      ...
      # No need to wrap the contents at all
        ...
        $("//ul[contains(concat(' ', @class, ' '), ' morecolors ')]/..") {
                       ...
                       $("//div[contains(concat(' ', @class, ' '), ' moreColorsBar ')]/..") {
    ...
    # No need to wrap the contents at all
      ...
      $("(//div[@id = 'localContentContainer'])[1]") {
        #remove banner
        $("div[@class='widget']"){
          remove()
        }
        
        $(".//div[@id='subAdBottomLong']"){
          remove()
        }
        var("id", fetch("./@id"))
        match($id, /^$/) {
  ...
  $("(//body[contains(concat(' ', @class, ' '), ' splash ')])[1]") {
    ...
    # No need to wrap the contents at all
      ...
      $("//ul[contains(concat(' ', @class, ' '), ' nav_cat_sub_2 ')]//li[contains(concat(' ', @class, ' '), ' nav_cat_item_bold ')]/div[contains(concat(' ', @class, ' '), ' mvDarkGrayBar ')]") {
                    inner() {
                      prepend("<div><span class=\"mvPlus\">+ </span><span class=\"mvMinus\">&minus; </span></div>")
                    }
                  }
      ...
      $("./a[contains(@href,'.com/shop') and not(contains(@href, 'gifts-gift-cards'))]") {
          attribute("href") {
           value() {
             ...
             append("%26!fn=sortBy%3DORIGINAL&!qvp=iqvp")
          }
        }
      }
    }
  ...
  # Removing the Beauty Blog link, since we're not supporting it anymore
  $("/html/body/div[@id='doc3']/div[@id='bd']/div[@id='globalContentContainer']/div[@id='localNavigationContainer']/ul[contains(@class,'nav_cat_sub_2')]/li/ul[contains(@class,'nav_cat_sub_3')]/li/a") {
    var("inner_text", fetch("./text()"))
    match($inner_text, /Beauty Blog/) {
      remove()
    }
  }
  
  $("/html/body//div[@class='ft errorMsgPanelFt']/div[@class='continueBtn']") {
    add_class("mvRedBtn")
  }
  
  # disable 'view all brands' so it doesn't behave like a facet (but behaves normally like a popup)
  $("/html/body//li[contains(@class, 'moreLinkContainer')]") {
    add_class("disabled")
  }

...
$("//body"){
  ...
  #  match($fake_url, not(/.*\.jsp/)) {
    ...
    $("//div[@class='mvNomobile']") {
      inject_bottom('<a class="desktop_site" target="_blank" href="">Go to Full HTML Version</a>')
...
# http://mlocal.macys.com/popup/r71x12_0420_area_rug.jsp
match($fake_url,/(area_rug\.jsp)/) {
  $("//body"){
    $(".//div"){
      attribute("style", "")
    }
    attribute("id", "mvRugBody")
    $(".//div[@class='mvNomobile']"){
      remove()
    }
    $(".//img[@title='close']/.."){
      add_class("mvCloseBtn")
    }
  }
}

# reviews.macys.com/content/7129/guidelines.htm
# reviews.macys.com/content/7129/termsandconditions.htm
match($fake_url,/(content\/7129\/(guidelines|termsandconditions)\.htm)/) {
  $("//body"){
    attribute("id", "mvReviewPopup")
  }
  }
  match($fake_url,/(\/catalog\/product\/pricingpolicy\.ognc)/) {
     $("html/body"){
       attribute("id", "mvPricePolicy")
       $("div[@id='doc3']"){
         $("div") {
           attribute("style", "")
           add_class("mvCloseBtn")

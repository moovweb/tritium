...
html() {
    ...
    $("body"){
      $("div[@id='doc3']"){
        $("div[@id='hd']"){
          ...
          $(".//p[@id='signInProfileInfo']/span[text()='|']") {
            remove()
          }
          $(".//div[@class='quickbagMasthead']") {
            move_here("../div[@id='quickBagHeader']/a", "top") {
  ...
  $("(//input[@name = \"KEYWORD_GO_BUTTON\"])[1]") {
    ...
    # No need to wrap the contents at all
      ...
      $("//*[@id = 'globalMastheadMiniCart']/*[not(@id='quickBagHeader')][1]") {
        # add_class("sprite_me-bag")
  ...
  match($element_exists, "false") {
    ...
    #check out Header Cart
    #Group::BasicGroup
       
                $("//div[@id='globalMastheadMiniCart']"){  
                  $("p/a/script[contains(text(), 'writeCartItems')]/..") {
                    add_class('globalMiniCartItems')
                  }
                $("p/a[contains(@class, 'globalMiniCartItems')]"){
                  move_to("..", "before")
                }
                $("p"){
                                                remove()
    ...
    # No need to wrap the contents at all
      ...
      $("(//*[@id = 'checkoutFooterContent']/strong)[1]") {
        ...
        inject_after("<div class='mvBottomNavContainer'><div class=\"mvBottomNav sprite_me-list_icon\"><span>Shop with Confidence</span></div></div>")
      }
  ...
  match($run_group, "true") {
    ...
    # No need to wrap the contents at all
      ...
      $("//*[contains(concat(' ', @class, ' '), ' mvFooterLinks ')]") {
                  ...
                  $("p[@id='signInProfileInfo']") {
                    attribute("style", "float:none; text-align:right; width:auto;")
                  }
                 }
      $("//*[contains(concat(' ', @class, ' '), ' moovweb_footer ')]") {
        remove()
  ...
  $("html/body") {
    ...
    insert_bottom("script") {
      attribute("language", "javascript")
      inner("window.addEventListener('DOMNodeInserted', function(i)  {var node = i.target;  var e = document.getElementById('mwCartItem');   var f = document.querySelector('#itemInfo b'); if (!f) return; f.innerHTML = f.innerHTML.replace(/(\\(|\\))/g, '');   if(e && f) {   e.innerHTML = f.innerHTML; } }, false);")
  ...
  $("html/body//a[contains(@class, 'globalMiniCartItems')]") {
    add_class("sprite_me-bag")
  }

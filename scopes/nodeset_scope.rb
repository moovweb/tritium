  ...
  class NodesetScope < Scope
    ...
    def select(selector, &block)
      ...
      if child_nodeset.size > 0
        child_scope = NodesetScope.new(child_nodeset, @root, self)
    ...
    def attribute(name, value = nil, &block)
      if name.is_a?(String)
        @nodeset.each do |node|
          ...
          if block
            attribute_scope = AttributeScope.new(attribute_node, @root, self)
    ...
    def insert_tag(tag_name, contents = "", attributes = {}, &block)
      ...
      @nodeset.each do |node|
        ...
        if block
          NodesetScope.new(new_node, @root, self).instance_eval(&block)
        end
      end
    end
    ...
    def move_to(selector, options = {}, &block)
      ...
      @nodeset.each do |node|
        ...
        if block
          NodesetScope.new(node.search("."), @root, self).instance_eval(&block)
        end
      end
    end
